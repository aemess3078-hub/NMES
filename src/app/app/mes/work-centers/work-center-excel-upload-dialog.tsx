...
import {
  importValidatedWorkCenters,
  validateWorkCenterExcelRows,
  type RawWorkCenterExcelRow,
  type ValidatedWorkCenterRow,
  type WorkCenterRowError,
} from "@/lib/actions/work-center-excel.actions"
import downloadWorkCenterTemplate from "./work-center-excel-download"

async function parseExcelFile(file: File): Promise<RawWorkCenterExcelRow[] | string> {
  ...
  const allRows = XLSX.utils.sheet_to_json<RawWorkCenterExcelRow>(ws, { defval: "", range: 1 })
  if (allRows.length === 0) return "데이터 행이 없습니다. 양식에 실제 데이터를 입력하세요."

  const dataRows = allRows.filter((row) => {
    const code = String(row["작업센터코드 *"] ?? row["작업센터코드"] ?? "").trim()
    return code !== "" && !code.startsWith("예:")
  })
  if (dataRows.length === 0) return "유효한 데이터 행이 없습니다."

  return dataRows.map((row) => {
    const normalized: RawWorkCenterExcelRow = {}
...
interface WorkCenterExcelUploadDialogProps {
  open: boolean
...
export function WorkCenterExcelUploadDialog({ open, onOpenChange }: WorkCenterExcelUploadDialogProps) {
  const router = useRouter()
  const fileRef = useRef<HTMLInputElement>(null)
  const [step, setStep] = useState<Step>("select")
  const [fileName, setFileName] = useState("")
  const [parseError, setParseError] = useState("")
  const [errors, setErrors] = useState<WorkCenterRowError[]>([])
  const [validRows, setValidRows] = useState<ValidatedWorkCenterRow[]>([])
  ...
  async function handleFileChange(event: React.ChangeEvent<HTMLInputElement>) {
    ...
    startTransition(async () => {
      const result = await validateWorkCenterExcelRows(parsed)
  ...
  function handleImport() {
    startTransition(async () => {
      const result = await importValidatedWorkCenters(validRows)
      if (result.success) {
        setImportMsg(`${result.importedCount}개 작업센터를 등록했습니다.`)
        setStep("done")
        router.refresh()
      } else {
  ...
  return (
    <Dialog open={open} onOpenChange={(nextOpen) => { if (!nextOpen) handleClose() }}>
      <DialogContent className="max-w-3xl">
        <DialogHeader>
          <DialogTitle>공정/작업장 엑셀 업로드</DialogTitle>
        </DialogHeader>
          ...
          <div className="space-y-4 py-2">
            <div className="flex items-center justify-between gap-4 rounded-lg border border-dashed bg-muted/20 p-4">
              <div className="flex items-center gap-3">
                ...
                <div>
                  <p className="text-[14px] font-medium">업로드 양식으로 작업센터를 일괄 등록합니다.</p>
                  <p className="text-[13px] text-muted-foreground">필수 컬럼은 * 표시가 있으며, 오류가 있으면 전체 등록이 차단됩니다.</p>
                </div>
              </div>
              <Button variant="outline" size="sm" onClick={downloadWorkCenterTemplate} className="shrink-0 gap-2">
                  ...
                  <table className="w-full text-[13px]">
                    <thead>
                      <tr className="border-b bg-muted/30">
                        ...
                        <th className="px-3 py-2 text-left font-medium text-muted-foreground">사업장</th>
                        <th className="px-3 py-2 text-left font-medium text-muted-foreground">코드</th>
                        <th className="px-3 py-2 text-left font-medium text-muted-foreground">작업센터명</th>
                    ...
                    <tbody>
                      {validRows.slice(0, 10).map((row) => (
                        <tr key={row.rowNum} className="border-b last:border-0">
                          ...
                          <td className="px-3 py-2">{kindLabels[row.kind]}</td>
                        </tr>
                      ))}
                    </tbody>
                  </table>
        ...
        {step === "done" && (
          <div className="flex flex-col items-center gap-3 py-8">
            ...
            <p className="text-[13px] text-muted-foreground">공정 목록을 새로고침했습니다.</p>
          </div>
        )}
          ...
          {step === "preview" && (
            <>
              ...
              <Button onClick={handleImport} disabled={!canImport || isPending} className="gap-1.5">
                ...
                {isPending ? "등록 중..." : `${validRows.length}개 작업센터 등록`}
              </Button>
            </>
          )}
