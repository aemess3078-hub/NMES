...
import {
  importValidatedRoutings,
  validateRoutingExcelRows,
  type RoutingRowError,
  type RawRoutingExcelRow,
  type ValidatedRoutingGroup,
} from "@/lib/actions/routing-excel.actions"
import { downloadRoutingTemplate } from "./routing-excel-download"

async function parseExcelFile(file: File): Promise<RawRoutingExcelRow[] | string> {
  ...
  const allRows = XLSX.utils.sheet_to_json<RawRoutingExcelRow>(ws, { defval: "", range: 1 })
  if (allRows.length === 0) return "데이터 행이 없습니다. 양식에 실제 데이터를 입력하세요."

  const dataRows = allRows.filter((row) => {
    const code = String(row["품목코드 *"] ?? row["품목코드"] ?? "").trim()
    return code !== "" && !code.startsWith("예:")
  })
  if (dataRows.length === 0) return "유효한 데이터 행이 없습니다."

  return dataRows.map((row) => {
    const normalized: RawRoutingExcelRow = {}
...
interface RoutingExcelUploadDialogProps {
  open: boolean
...
export function RoutingExcelUploadDialog({ open, onOpenChange }: RoutingExcelUploadDialogProps) {
  const router = useRouter()
  const fileRef = useRef<HTMLInputElement>(null)
  const [step, setStep] = useState<Step>("select")
  const [fileName, setFileName] = useState("")
  const [parseError, setParseError] = useState("")
  const [errors, setErrors] = useState<RoutingRowError[]>([])
  const [validGroups, setValidGroups] = useState<ValidatedRoutingGroup[]>([])
  ...
  async function handleFileChange(event: React.ChangeEvent<HTMLInputElement>) {
    ...
    startTransition(async () => {
      const result = await validateRoutingExcelRows(parsed)
  ...
  function handleImport() {
    startTransition(async () => {
      const result = await importValidatedRoutings(validGroups)
      if (result.success) {
        setImportMsg(`라우팅 ${result.importedRoutingCount}개 (공정 ${result.importedOperationCount}행)를 등록했습니다.`)
        setStep("done")
        router.refresh()
      } else {
  ...
  const totalOpCount = validGroups.reduce((sum, g) => sum + g.operations.length, 0)

  return (
    <Dialog open={open} onOpenChange={(nextOpen) => { if (!nextOpen) handleClose() }}>
      <DialogContent className="max-w-3xl">
        <DialogHeader>
          <DialogTitle>라우팅 엑셀 업로드</DialogTitle>
        </DialogHeader>
          ...
          <div className="space-y-4 py-2">
            <div className="flex items-center justify-between gap-4 rounded-lg border border-dashed bg-muted/20 p-4">
              <div className="flex items-center gap-3">
                ...
                <div>
                  <p className="text-[14px] font-medium">업로드 양식으로 라우팅을 일괄 등록합니다.</p>
                  <p className="text-[13px] text-muted-foreground">같은 라우팅코드의 행들이 하나의 라우팅을 구성합니다.</p>
                </div>
              </div>
              <Button variant="outline" size="sm" onClick={downloadRoutingTemplate} className="shrink-0 gap-2">
          ...
          <div className="space-y-4 py-2">
            <div className="grid grid-cols-4 gap-3">
              ...
              <div className="rounded-lg border border-emerald-200 bg-emerald-50 p-3 text-center">
                <p className="text-[13px] text-emerald-700">정상 라우팅</p>
                <p className="text-[22px] font-bold text-emerald-700">{validGroups.length}</p>
              </div>
              <div className="rounded-lg border border-emerald-200 bg-emerald-50 p-3 text-center">
                <p className="text-[13px] text-emerald-700">정상 공정 행</p>
                <p className="text-[22px] font-bold text-emerald-700">{totalOpCount}</p>
              </div>
            ...
            {validGroups.length > 0 && (
              <div>
                <p className="mb-2 flex items-center gap-1.5 text-[14px] font-medium">
                  ...
                  등록 예정 라우팅 (최대 5건 미리보기)
                </p>
                <div className="overflow-hidden rounded-lg border">
                  <table className="w-full text-[13px]">
                    <thead>
                      <tr className="border-b bg-muted/30">
                        <th className="px-3 py-2 text-left font-medium text-muted-foreground">품목코드</th>
                        <th className="px-3 py-2 text-left font-medium text-muted-foreground">라우팅코드</th>
                        <th className="px-3 py-2 text-left font-medium text-muted-foreground">라우팅명</th>
                        <th className="px-3 py-2 text-left font-medium text-muted-foreground">버전</th>
                        <th className="px-3 py-2 text-left font-medium text-muted-foreground">공정 수</th>
                    ...
                    <tbody>
                      {validGroups.slice(0, 5).map((group) => (
                        <tr key={group.key} className="border-b last:border-0">
                          <td className="px-3 py-2 font-mono">{group.itemCode}</td>
                          <td className="px-3 py-2 font-mono">{group.routingCode}</td>
                          <td className="px-3 py-2">{group.routingName}</td>
                          <td className="px-3 py-2">{group.version}</td>
                          <td className="px-3 py-2">{group.operations.length}개</td>
                  ...
                  {validGroups.length > 5 && (
                    <p className="bg-muted/10 px-3 py-2 text-[13px] text-muted-foreground">
                      외 {validGroups.length - 5}개 라우팅 더 있음
                    </p>
                  )}
                </div>
              </div>
        ...
        {step === "done" && (
          <div className="flex flex-col items-center gap-3 py-8">
            ...
            <p className="text-[13px] text-muted-foreground">라우팅 목록을 새로고침했습니다.</p>
          </div>
        )}
          ...
          {step === "preview" && (
            <>
              ...
              <Button onClick={handleImport} disabled={!canImport || isPending} className="gap-1.5">
                ...
                {isPending ? "등록 중..." : `라우팅 ${validGroups.length}개 등록`}
              </Button>
            </>
          )}
