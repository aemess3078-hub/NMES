"use server"

import { prisma } from "@/lib/db/prisma"
import { requireRole, getTenantId } from "@/lib/auth"
import { revalidatePath } from "next/cache"
import { resolveKstDateRangeFilter } from "@/lib/date/kst"
import { getErrorMessage } from "@/lib/utils"
import {
  normalizeActionContent,
  normalizeCompletionNote,
  normalizeDueDate,
  buildCorrectiveActionStatusWhere,
  serializeDefectCorrectiveActionRow,
  type CorrectiveActionStatusFilter,
  type DefectCorrectiveActionRow,
} from "./defect-corrective-action.helpers"

export type { DefectCorrectiveActionRow }

// ─── 청운커팅 사업계획서 "품질검사 > 조치관리" ──────────────────────────────
//
// 불량발생 → 원인분석(기존 PR #55, 이 파일에서는 참조만 하고 절대 수정하지
// 않는다) → 조치 등록 → 담당자 지정 → 완료기한 관리 → 조치 진행 → 조치 완료
// 구간을 담당한다. 재발방지관리(효과성 검증/재발여부평가/CAPA 종료판정)는
// 별도 기능/PR로 분리하며, 이 파일에는 관련 필드/로직을 추가하지 않는다.
// tenantId/defectRecordId/assigneeId/actionId는 client가 보낸 값을 신뢰하지
// 않고 서버에서 항상 재검증한다(defect-cause-analysis.actions.ts와 동일 원칙).

const MENU_NAME = "조치관리"

function revalidateCorrectiveActionPaths() {
  revalidatePath("/app/mes/quality/corrective-action")
}

/** client가 보낸 defectRecordId는 신뢰하지 않고, 현재 tenant 소속인지 서버에서 재검증한다. */
async function assertDefectRecordInTenant(defectRecordId: string, tenantId: string) {
  const record = await prisma.defectRecord.findFirst({
    where: {
      id: defectRecordId,
      qualityInspection: { workOrderOperation: { workOrder: { tenantId } } },
    },
    select: { id: true },
  })
  if (!record) throw new Error("불량 기록을 찾을 수 없습니다.")
}

/** 담당자 지정 시 현재 tenant의 활성 TenantUser에 속한 Profile만 허용한다(project-issue.actions.ts와 동일 패턴). */
async function assertAssigneeValid(tenantId: string, assigneeId: string | null | undefined) {
  if (!assigneeId) return
  const tenantUser = await prisma.tenantUser.findFirst({
    where: { profileId: assigneeId, tenantId, isActive: true },
    select: { profileId: true },
  })
  if (!tenantUser) {
    throw new Error("담당자를 찾을 수 없습니다. 활성 상태인 담당자만 지정할 수 있습니다.")
  }
}

const CORRECTIVE_ACTION_SELECT = {
  id: true,
  actionContent: true,
  assigneeId: true,
  assignee: { select: { name: true } },
  dueDate: true,
  status: true,
  completedAt: true,
  completionNote: true,
  createdBy: { select: { name: true } },
  updatedBy: { select: { name: true } },
  createdAt: true,
  updatedAt: true,
  defectRecord: {
    select: {
      id: true,
      qty: true,
      severity: true,
      disposition: true,
      defectCode: { select: { code: true, name: true } },
      causeAnalysis: { select: { rootCause: true, analysisDetail: true } },
      qualityInspection: {
        select: {
          inspectedAt: true,
          stage: true,
          workOrderOperation: {
            select: {
              routingOperation: { select: { name: true } },
              workOrder: {
                select: {
                  orderNo: true,
                  manufacturingNo: true,
                  item: { select: { code: true, name: true } },
                },
              },
            },
          },
        },
      },
    },
  },
} as const

// ─── 목록 조회 ────────────────────────────────────────────────────────────────

export type DefectCorrectiveActionFilter = {
  from?: string // YYYY-MM-DD(KST) — 대상 불량의 검사일시 기준(원인분석 화면과 동일 기준)
  to?: string
  itemId?: string
  routingOperationId?: string
  manufacturingNo?: string
  defectCodeId?: string
  assigneeId?: string
  status?: CorrectiveActionStatusFilter
}

async function getDefectCorrectiveActionRecords(tenantId: string, filter: DefectCorrectiveActionFilter) {
  const { fromDate, toDate } = resolveKstDateRangeFilter(30, filter.from, filter.to)

  const workOrderWhere: Record<string, unknown> = { tenantId }
  if (filter.itemId) workOrderWhere.itemId = filter.itemId
  if (filter.manufacturingNo?.trim()) workOrderWhere.manufacturingNo = filter.manufacturingNo.trim()

  const workOrderOperationWhere: Record<string, unknown> = { workOrder: workOrderWhere }
  if (filter.routingOperationId) workOrderOperationWhere.routingOperationId = filter.routingOperationId

  return prisma.defectCorrectiveAction.findMany({
    where: {
      tenantId,
      defectRecord: {
        qualityInspection: {
          inspectedAt: { gte: fromDate, lte: toDate },
          workOrderOperation: workOrderOperationWhere,
        },
        ...(filter.defectCodeId && { defectCodeId: filter.defectCodeId }),
      },
      ...(filter.assigneeId && { assigneeId: filter.assigneeId }),
      ...buildCorrectiveActionStatusWhere(filter.status),
    },
    select: CORRECTIVE_ACTION_SELECT,
    orderBy: [{ dueDate: "asc" }, { id: "asc" }],
  })
}

export async function getDefectCorrectiveActionList(
  filter: DefectCorrectiveActionFilter = {}
): Promise<DefectCorrectiveActionRow[]> {
  await requireRole("VIEWER")
  const tenantId = await getTenantId()
  const records = await getDefectCorrectiveActionRecords(tenantId, filter)
  const now = new Date()
  const rows = records.map((r) => serializeDefectCorrectiveActionRow(r, now))
  // "OVERDUE"는 DB status가 아니라 계산값이라, where절이 아닌 여기서 걸러낸다.
  if (filter.status === "OVERDUE") return rows.filter((r) => r.overdue)
  return rows
}

// ─── 필터 옵션 / 담당자 선택지 ────────────────────────────────────────────────

export type DefectCorrectiveActionFilterOptions = {
  items: { id: string; code: string; name: string }[]
  routingOperations: { id: string; name: string; seq: number; routingCode: string; routingName: string }[]
  defectCodes: { id: string; code: string; name: string }[]
  assignableUsers: { id: string; name: string }[]
}

export async function getDefectCorrectiveActionFilterOptions(): Promise<DefectCorrectiveActionFilterOptions> {
  await requireRole("VIEWER")
  const tenantId = await getTenantId()

  const [items, ops, defectCodes, tenantUsers] = await Promise.all([
    prisma.item.findMany({
      where: { tenantId, status: "ACTIVE" },
      select: { id: true, code: true, name: true },
      orderBy: { code: "asc" },
    }),
    prisma.routingOperation.findMany({
      where: { routing: { tenantId } },
      select: { id: true, name: true, seq: true, routing: { select: { code: true, name: true } } },
      orderBy: [{ routing: { code: "asc" } }, { seq: "asc" }],
    }),
    prisma.defectCode.findMany({
      where: { tenantId },
      select: { id: true, code: true, name: true },
      orderBy: { code: "asc" },
    }),
    prisma.tenantUser.findMany({
      where: { tenantId, isActive: true },
      select: { profileId: true, profile: { select: { name: true } } },
      orderBy: { profile: { name: "asc" } },
    }),
  ])

  return {
    items,
    routingOperations: ops.map((op) => ({
      id: op.id,
      name: op.name,
      seq: op.seq,
      routingCode: op.routing.code,
      routingName: op.routing.name,
    })),
    defectCodes,
    assignableUsers: tenantUsers.map((u) => ({ id: u.profileId, name: u.profile.name })),
  }
}

// ─── 조치 등록 대상 불량 선택지 ───────────────────────────────────────────────
//
// 원인분석 화면과 동일한 대상 범위(기간 내 DefectRecord 전체)를 재사용한다 —
// 원인분석이 아직 없는 불량도 조치 등록은 가능하게 하되(원인분석 선행을
// 강제하지 않음, §5), rootCause/analysisDetail이 있으면 등록 폼에서 함께
// 보여준다.

export type CorrectiveActionDefectOption = {
  defectRecordId: string
  inspectedAt: string
  itemCode: string
  itemName: string
  routingOperationName: string
  orderNo: string
  manufacturingNo: string | null
  defectCode: string
  defectCodeName: string
  defectQty: number
  rootCause: string | null
  analysisDetail: string | null
}

export async function getCorrectiveActionDefectOptions(
  filter: { from?: string; to?: string } = {}
): Promise<CorrectiveActionDefectOption[]> {
  await requireRole("VIEWER")
  const tenantId = await getTenantId()
  const { fromDate, toDate } = resolveKstDateRangeFilter(30, filter.from, filter.to)

  const records = await prisma.defectRecord.findMany({
    where: {
      qualityInspection: {
        inspectedAt: { gte: fromDate, lte: toDate },
        workOrderOperation: { workOrder: { tenantId } },
      },
    },
    select: {
      id: true,
      qty: true,
      defectCode: { select: { code: true, name: true } },
      causeAnalysis: { select: { rootCause: true, analysisDetail: true } },
      qualityInspection: {
        select: {
          inspectedAt: true,
          workOrderOperation: {
            select: {
              routingOperation: { select: { name: true } },
              workOrder: {
                select: {
                  orderNo: true,
                  manufacturingNo: true,
                  item: { select: { code: true, name: true } },
                },
              },
            },
          },
        },
      },
    },
    orderBy: [{ qualityInspection: { inspectedAt: "desc" } }, { id: "asc" }],
  })

  return records.map((r) => {
    const woOp = r.qualityInspection.workOrderOperation
    return {
      defectRecordId: r.id,
      inspectedAt: r.qualityInspection.inspectedAt.toISOString(),
      itemCode: woOp.workOrder.item.code,
      itemName: woOp.workOrder.item.name,
      routingOperationName: woOp.routingOperation.name,
      orderNo: woOp.workOrder.orderNo,
      manufacturingNo: woOp.workOrder.manufacturingNo,
      defectCode: r.defectCode.code,
      defectCodeName: r.defectCode.name,
      defectQty: Number(r.qty),
      rootCause: r.causeAnalysis?.rootCause ?? null,
      analysisDetail: r.causeAnalysis?.analysisDetail ?? null,
    }
  })
}

// ─── 등록 ───────────────────────────────────────────────────────────────────

export type CreateDefectCorrectiveActionInput = {
  defectRecordId: string
  actionContent: string
  assigneeId?: string | null
  dueDate: string
}

export async function createDefectCorrectiveAction(
  data: CreateDefectCorrectiveActionInput
): Promise<{ id: string }> {
  const actor = await requireRole("OPERATOR")
  const tenantId = await getTenantId()

  const actionContent = normalizeActionContent(data.actionContent)
  const dueDate = normalizeDueDate(data.dueDate)
  const assigneeId = data.assigneeId?.trim() || null

  await assertDefectRecordInTenant(data.defectRecordId, tenantId)
  await assertAssigneeValid(tenantId, assigneeId)

  const created = await prisma.$transaction(async (tx) => {
    const action = await tx.defectCorrectiveAction.create({
      data: {
        tenantId,
        defectRecordId: data.defectRecordId,
        actionContent,
        assigneeId,
        dueDate,
        status: "OPEN",
        createdById: actor.id,
        updatedById: actor.id,
      },
    })
    await tx.auditLog.create({
      data: {
        tenantId,
        actorId: actor.id,
        actorLabel: actor.name,
        entityType: "DefectCorrectiveAction",
        entityId: action.id,
        action: "CREATE",
        afterData: {
          actionContent: action.actionContent,
          assigneeId: action.assigneeId,
          dueDate: action.dueDate,
          status: action.status,
        },
        menuName: MENU_NAME,
      },
    })
    return action
  })

  revalidateCorrectiveActionPaths()
  return { id: created.id }
}

// ─── 수정 (조치내용 / 담당자 / 완료예정일) ───────────────────────────────────
//
// 상태 자체는 이 함수로 바꾸지 않는다 — 상태전이는 아래 start/complete 전용
// 함수로만 처리한다(project-issue.actions.ts와 동일하게 책임을 분리).

export type UpdateDefectCorrectiveActionInput = {
  actionContent: string
  assigneeId?: string | null
  dueDate: string
}

export async function updateDefectCorrectiveAction(id: string, data: UpdateDefectCorrectiveActionInput) {
  const actor = await requireRole("OPERATOR")
  const tenantId = await getTenantId()

  const existing = await prisma.defectCorrectiveAction.findFirst({ where: { id, tenantId } })
  if (!existing) throw new Error("조치 이력을 찾을 수 없습니다.")

  const actionContent = normalizeActionContent(data.actionContent)
  const dueDate = normalizeDueDate(data.dueDate)
  const assigneeId = data.assigneeId?.trim() || null
  await assertAssigneeValid(tenantId, assigneeId)

  await prisma.$transaction(async (tx) => {
    await tx.defectCorrectiveAction.update({
      where: { id },
      data: { actionContent, assigneeId, dueDate, updatedById: actor.id },
    })
    await tx.auditLog.create({
      data: {
        tenantId,
        actorId: actor.id,
        actorLabel: actor.name,
        entityType: "DefectCorrectiveAction",
        entityId: id,
        action: "UPDATE",
        beforeData: {
          actionContent: existing.actionContent,
          assigneeId: existing.assigneeId,
          dueDate: existing.dueDate,
        },
        afterData: { actionContent, assigneeId, dueDate },
        menuName: MENU_NAME,
      },
    })
  })

  revalidateCorrectiveActionPaths()
}

// ─── 상태전이: 조치 진행 (OPEN → IN_PROGRESS) ────────────────────────────────

export async function startDefectCorrectiveAction(id: string): Promise<{ ok: boolean; error?: string }> {
  try {
    const actor = await requireRole("OPERATOR")
    const tenantId = await getTenantId()

    const current = await prisma.defectCorrectiveAction.findFirst({ where: { id, tenantId } })
    if (!current) throw new Error("조치 이력을 찾을 수 없습니다.")
    if (current.status !== "OPEN") throw new Error("등록 상태인 조치만 진행을 시작할 수 있습니다.")

    await prisma.$transaction(async (tx) => {
      const claimed = await tx.defectCorrectiveAction.updateMany({
        where: { id: current.id, tenantId, status: "OPEN" },
        data: { status: "IN_PROGRESS", updatedById: actor.id },
      })
      if (claimed.count !== 1) {
        throw new Error("다른 요청에 의해 상태가 이미 변경되었습니다. 새로고침 후 다시 시도해 주세요.")
      }
      await tx.auditLog.create({
        data: {
          tenantId,
          actorId: actor.id,
          actorLabel: actor.name,
          entityType: "DefectCorrectiveAction",
          entityId: current.id,
          action: "UPDATE",
          beforeData: { status: "OPEN" },
          afterData: { status: "IN_PROGRESS" },
          menuName: MENU_NAME,
        },
      })
    })

    revalidateCorrectiveActionPaths()
    return { ok: true }
  } catch (e) {
    return { ok: false, error: getErrorMessage(e) }
  }
}

// ─── 상태전이: 조치 완료 (IN_PROGRESS → COMPLETED) ───────────────────────────

export async function completeDefectCorrectiveAction(
  id: string,
  completionNote?: string | null
): Promise<{ ok: boolean; error?: string }> {
  try {
    const actor = await requireRole("OPERATOR")
    const tenantId = await getTenantId()

    const current = await prisma.defectCorrectiveAction.findFirst({ where: { id, tenantId } })
    if (!current) throw new Error("조치 이력을 찾을 수 없습니다.")
    if (current.status === "COMPLETED") throw new Error("이미 완료된 조치입니다.")
    if (current.status !== "IN_PROGRESS") throw new Error("진행중인 조치만 완료 처리할 수 있습니다.")

    const note = normalizeCompletionNote(completionNote)

    await prisma.$transaction(async (tx) => {
      const completedAt = new Date()
      const claimed = await tx.defectCorrectiveAction.updateMany({
        where: { id: current.id, tenantId, status: "IN_PROGRESS" },
        data: { status: "COMPLETED", completedAt, completionNote: note, updatedById: actor.id },
      })
      if (claimed.count !== 1) {
        throw new Error("다른 요청에 의해 상태가 이미 변경되었습니다. 새로고침 후 다시 시도해 주세요.")
      }
      await tx.auditLog.create({
        data: {
          tenantId,
          actorId: actor.id,
          actorLabel: actor.name,
          entityType: "DefectCorrectiveAction",
          entityId: current.id,
          action: "UPDATE",
          beforeData: { status: "IN_PROGRESS" },
          afterData: { status: "COMPLETED", completedAt, completionNote: note },
          menuName: MENU_NAME,
        },
      })
    })

    revalidateCorrectiveActionPaths()
    return { ok: true }
  } catch (e) {
    return { ok: false, error: getErrorMessage(e) }
  }
}
